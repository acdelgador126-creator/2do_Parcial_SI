...
namespace App\Http\Controllers\RegistroPostulantes;
...
use App\Services\RegistroPostulantes\VerificacionExternaService;
...
class PostulanteController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($existente) {
            DB::transaction(function () use ($existente, $validated, $gestion) {
                ...
                \App\Models\Evaluacion\Examen::where('postulante_id', $existente->id)->delete();
                \App\Models\Evaluacion\NotaFinal::where('postulante_id', $existente->id)->delete();
                \App\Models\PlanificacionAcademica\AsignacionGrupo::where('postulante_id', $existente->id)->delete();
                \App\Models\AdmisionCarreras\AdmisionCarreras\Admision::where('postulante_id', $existente->id)->delete();
                ...
                $user = \App\Models\Autenticacion\User::where('email', $existente->email)->first();
                if ($user) {
    ...
    public function deleteByEmail(Request $request): JsonResponse
    {
        ...
        $user = \App\Models\Autenticacion\User::where('email', $validated['email'])->first();
        if ($user) {
            $user->delete();
        }
        ...
        // Eliminar pagos
        \App\Models\RegistroPostulantes\Pago::where('postulante_id', $postulante->id)->delete();

        // Eliminar asignaciones de grupo
        \App\Models\PlanificacionAcademica\AsignacionGrupo::where('postulante_id', $postulante->id)->delete();

        // Eliminar notificaciones del usuario asociado
        if ($postulante->user_id) {
            \App\Models\ReportesIA\Notificacion::where('usuario_id', $postulante->user_id)->delete();
        }
